...
def save_recording(save_or_not, SAMPLE_RATE, audio_recording): #This processes whether the voice file will be saved or discarded
    if save_or_not in ["yes", "y"]:
        custom_voice_file_name = input("Do you want to use a custom name for the file? Please select yes or no. ") #Option for user to insert their own name, instead of date and time as file name(s)
        if custom_voice_file_name in ["yes", "y"]:
            voice_file_name = (input("Please insert a name for the voice file: ")) + ".wav"
        else:
...
def recording_to_text(voice_file_name):
    ...
    with sr.AudioFile(voice_file_name) as source: #This will open the file
        ...
        text = r.recognize_google(audio_data, language = "en-US", key=None, show_all=True) #convert from speech to text
        return text
